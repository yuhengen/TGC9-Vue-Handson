<template>
  <div class="container">
    <div class="d-flex flex-wrap">
      <!-- <div class="card task" style="width: 18rem" v-for="t in tasks" v-bind:key="t.id">
      <div class="card-body">
        <h5 class="card-title">{{t.name}}</h5>
        <p class="card-text">
          <ul>
              <li>Urgency: {{t.urgency}}</li>
              <li>Due Date: {{t.date_due}}</li>
              <li>Done: {{t.done}}</li>
          </ul>
        </p>
      ...
      <!-- Splitting the components so it can be reused -->
      <Task
        v-for="t in tasks"
        :task="t"
        :key="t.id"
        @delete_task="processDeleteTask"
        @task_updated="processUpdateTask"
      />
    </div>
    <div>
      <div>
